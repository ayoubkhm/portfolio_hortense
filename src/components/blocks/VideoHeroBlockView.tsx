...
// Client component for the video-hero block. Fetches the video path at
// runtime from /api/settings/hero-video (the existing endpoint that handles
// the homepage video upload), so the video URL doesn't need to be stored
...
export default function VideoHeroBlockView({ block }: { block: VideoHeroBlock }) {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-4 text-center">
        <h1 className="font-serif text-5xl font-bold text-white md:text-7xl">{block.data.title}</h1>

        {block.data.subtitle && (
          <p className="mt-4 text-lg tracking-wide text-sand md:text-xl">{block.data.subtitle}</p>
        )}
        {block.data.subSubtitle && (
          <p className="mt-2 text-base tracking-wide text-sand/80 md:text-lg">{block.data.subSubtitle}</p>
        )}
